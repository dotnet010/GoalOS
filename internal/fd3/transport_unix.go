...
func Listen(dir, label string) (*Listener, error) {
	...
	path := filepath.Join(dir, fmt.Sprintf("goalos-fd3-%s-%s.sock", sanitizeLabel(label), hex.EncodeToString(entropy[:])))
	...
	return &Listener{base: path, ln: ln}, nil
}
