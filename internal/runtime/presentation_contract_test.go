...
// 四平台快照+grep 断言=人类可读通道零工程词汇（R-1512① 范围=人类可读通道；
...
// ①四平台快照（darwin/linux/windows/xinchuang）——产品词呈现；
// ②人类可读通道零工程词汇（黑名单 grep）；
// ③macOS 措辞（R-1479——纵深防御+弃用组件诚实标注）；
// ④行 2 警告样式标记（R-1550）；⑤拒绝句（行 5/6 合一——不虚构依据）；
// ⑥未知档位 fail-closed 最低呈现。
func TestRuntime_CLI_HonestTierPresentation(t *testing.T) {
	...
	// ⑤拒绝句（i4_backend_unavailable→行 5/6 合一人类句）
