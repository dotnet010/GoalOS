...
// ①名单认证→解析行 1→T0+matched_workload 短码（R-1589 读取点）；
// ②能力代理白名单：声明集外执行=代理拒绝（ProxyRefusals=1——不计入边界证据 TC-RT-002）；
// ③声明集内=真实执行（边界内——process.exec 机制层）；
// ④无契约租约=fail-closed（R-1501 契约不过边界）。
func TestRuntime_T0Provider_CapabilityProxy(t *testing.T) {
	ctx := context.Background()
	home := t.TempDir()

	// ①名单认证→行 1→T0（TC-RT-010 联动——名单条目+命中哈希）
	...
	if err != nil || sel.Tier != TierT0 {
		t.Fatalf("①名单认证应=T0 行 1，实际: %v err=%v", sel.Tier, err)
	}
	if sel.MatchedWorkload != "abababab" {
		t.Fatalf("①matched_workload 应=publisher_key 前 8 字符（R-1589），实际 %q", sel.MatchedWorkload)
	}
	...
	if err == nil || !strings.Contains(err.Error(), "能力代理拒绝") {
		t.Fatalf("②声明集外应=能力代理拒绝，实际: %v", err)
	}
	ch := h.(*collaborativeHandle)
	if ch.ProxyRefusals() != 1 {
		t.Fatalf("②ProxyRefusals 应=1，实际 %d", ch.ProxyRefusals())
	}

	// ③声明集内（fs.read）=放行进入 OS 边界执行
	res, err := h.Execute(ctx, ExecuteRequest{
		ActionID: "a-allow", ActionType: "fs.read",
		Params: map[string]string{"binary": "/usr/bin/true"},
	})
	if err != nil || res.Status != "success" {
		t.Fatalf("③声明集内应真实执行成功，实际: status=%s err=%v", res.Status, err)
	}
	if ch.ProxyRefusals() != 1 {
		t.Fatalf("③放行不得误计代理拒绝——计数应仍为 1，实际 %d", ch.ProxyRefusals())
	}

	// ④无契约租约=fail-closed
	if _, err := p.Acquire(ctx, LeaseRequest{GoalID: "g-t0", ActionID: "a2"}); err == nil {
		t.Fatal("④无契约租约必须拒绝（R-1501）")
	}
