...
//
// 契约（05 §3.18 权威）：两层检测——(1)确定性层零误报硬契约（工具调用集 vs 目标声明集
// 偏离+循环检测+预算异常+金丝雀信号复用 R-1080 联动）(2)语义层统计遥测（ESN+CUSUM
...
// Check — 漂移检测（两层检测）。
// (1)确定性层（零误报硬契约——工具调用集 vs 目标声明集偏离+循环检测+预算异常+金丝雀信号复用）
// (2)语义层（ESN+CUSUM 统计遥测模式检测）。
