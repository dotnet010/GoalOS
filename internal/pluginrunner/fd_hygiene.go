...
//
// 契约（R-928/R-1086 四防线）：(1)创建期 O_CLOEXEC CI AST (2)启动期 close_range+4.19
// /proc 兜底（CPython bpo-47260 无条件 fallback 教训）(3)spawn 期唯一入口 (4)验证期
...
// 契约：spawn 期唯一入口+子进程 fd 仅 {0,1,2,FD3}（验证期对抗契约 R-1086-4）。
