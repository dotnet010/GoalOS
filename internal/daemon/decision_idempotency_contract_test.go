...
// 断言：①新裁决=200+F-04 形状（{ok, goal_id, visible_state, updated_at}——R-1324 收口）；
// ②重复同决策=200 幂等回放（replayed=true——回放缓存原结果，Stripe 族）；
// ③重复冲突决策=409（Kees 硬语义——决策不可覆写）；④真未知=404（诚实回答）；
// ⑤并发双裁决=恰好一次生效（delete-in-lock 并发安全语义不变）。
package daemon_test
...
func TestDecision_IdempotencyThreeForms(t *testing.T) {
	...
	if w1.Code != http.StatusOK {
		t.Fatalf("①新裁决应=200，实际 %d", w1.Code)
	}
	...
	for _, f := range []string{"ok", "goal_id", "visible_state", "updated_at"} {
		if _, ok := body1[f]; !ok {
			t.Fatalf("①F-04 形状缺字段 %q（R-1324）", f)
		}
	}
	if body1["replayed"] != false {
		t.Fatal("①新裁决 replayed 必须=false")
	}
	...
	if w2.Code != http.StatusOK {
		t.Fatalf("②重复同决策应=200，实际 %d", w2.Code)
	}
	var body2 map[string]interface{}
	_ = json.Unmarshal(w2.Body.Bytes(), &body2)
	if body2["replayed"] != true {
		t.Fatal("②回放必须标记 replayed=true（审计可读=非重复执行证据）")
	}
	if body2["decision"] != "approved" {
		t.Fatalf("②回放应=原决策 approved，实际 %v", body2["decision"])
	}
	...
	if w3.Code != http.StatusConflict {
		t.Fatalf("③冲突决策应=409，实际 %d", w3.Code)
	}
	...
	if w4.Code != http.StatusNotFound {
		t.Fatalf("④真未知应=404，实际 %d", w4.Code)
	}

	// ⑤并发双裁决=恰好一次生效（delete-in-lock 语义不变）
	...
	if okCount != 8 {
		t.Fatalf("⑤并发裁决全部应=200（回放族），实际 200 数=%d/%d", okCount, len(codes))
	}
}
