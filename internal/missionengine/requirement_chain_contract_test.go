...
// (1)首版创建（无契约时需求注入=契约创建入口）；(2)修订链（version 递增+supersedes 连续）；
// (3)CompletionContractRecorded 落账（载荷校验过——R-770）；(4)重规划触发留痕。
func TestRequirementAdded_ConsumerChain(t *testing.T) {
	...
	case p := <-recorded:
		if p["version"] != 1 || p["status"] != "active" {
			t.Fatalf("(1)首版应=version 1/active，实际: %v", p)
		}
	case <-time.After(5 * time.Second):
		t.Fatal("(1)CompletionContractRecorded 未落账（消费链断裂）")
	}
	select {
	case p := <-replan:
		if p["reason"] != "requirement_added" {
			t.Fatalf("(4)重规划 reason 应=requirement_added（R-1400 透传），实际: %v", p["reason"])
		}
	case <-time.After(5 * time.Second):
		t.Fatal("(4)重规划未触发（PlanRequested 未发布）")
	}

	// (2)(3)修订链：第二次注入→version 2+revised+supersedes 连续
	bus.Publish(events.Event{
		Type: events.TypeRequirementAdded, GoalID: "goal-c1", Source: "test",
	...
	case p := <-recorded:
		if p["version"] != 2 || p["status"] != "revised" {
			t.Fatalf("(2)修订版应=version 2/revised，实际: %v", p)
		}
		if p["supersedes"] == "" || p["supersedes"] == nil {
			t.Fatal("(3)supersedes 必须携带旧版本契约 ID（线性链连续）")
		}
	case <-time.After(5 * time.Second):
		t.Fatal("(2)修订版未落账")
	}
