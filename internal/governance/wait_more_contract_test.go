...
// 断言：(1)延期接受+计时器重启（Temporal snooze——取消旧+新完整窗口）；(2)extensions_used 计数；
// (3)上限=3 次第 4 次=GOV-APX-F-001 即时返回（计时器继续——上限≠立即拒绝——R-1603 逐字）；
// (4)已裁决项 wait_more=不接受（幂等无副作用）；(5)时序实证：延期后超时点在延后期满（非原窗口）。
...
func TestGovernance_WaitMore_Extension(t *testing.T) {
	...
	if !accepted || used != 1 || exhausted {
		t.Fatalf("(1)首次延期应=接受+used=1+未超限，实际: accepted=%v used=%d exhausted=%v", accepted, used, exhausted)
	}

	// (3)上限：第 2/3 次接受，第 4 次=超限即时返回（计时器继续——R-1603）
	e.WaitMore("act-wm")
	e.WaitMore("act-wm")
	accepted4, used4, exhausted4 := e.WaitMore("act-wm")
	if accepted4 || !exhausted4 || used4 != 3 {
		t.Fatalf("(3)第 4 次应=超限即时返回（accepted=false exhausted=true used=3），实际: %v/%v/%d", accepted4, exhausted4, used4)
	}

	// (4)已裁决/未知项=不接受（幂等无副作用——D-3 三形态同族）
	acceptedX, _, _ := e.WaitMore("act-never")
	if acceptedX {
		t.Fatal("(4)未知审批项 wait_more 必须不接受")
	}

	// (5)时序实证：延期后超时点=延后期满（非原窗口）——秒级窗口纪律
	bus2 := eventbus.New()
	e2 := New(bus2, nil)
	e2.Start()
	...
	select {
	case <-rejected:
		t.Fatal("(5)原窗口期内（<3.2s）不应拒绝——延期未生效")
	...
	case <-time.After(2500 * time.Millisecond):
		t.Fatal("(5)延长期满（≈3.2s）后必须超时兜底拒绝（审批必须终结——R-1603）")
	}
}
