...
依据: 会议 #195 R-1099（Semantic Freeze）
EOF
}
...
echo "── ${BOLD}废弃语义残留检测${NC}（Semantic Freeze，模式 ${#DEPRECATED_PATTERNS[@]} 条）──"
...
if [ "$FAILED" -gt 0 ]; then
    echo "[ERROR] 活动规范文档正文存在废弃语义残留——旧语义复活风险。请按 Contract Authority 变更流程替换为新语义（R-1099 Semantic Freeze）。" >&2
    exit 1
fi
