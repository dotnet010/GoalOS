...
#     (1) docs/stub/**          ← 本布局=开发文档/stub追踪清单.md（stub 归档索引——历史行
#                               逐字封存「某语义已于某会议废弃」，机械命中=语义冻结的假阳性）
#     (2) docs/resolutions/**   ← 本布局=resolutions.yaml（决议注册表；不经本脚本扫描，
#                               路径模式登记=防未来扩围）
#     (3) red-evidence/**       ← 本布局=scripts/red-evidence/（先红/转绿存证 R-1632；
#                               同理不经本脚本扫描，登记同上）
...
# R-1656（会议 #262——顾问复审(5)采纳加牙）: Windows+真I3/已达I3/双面齐备 组合禁称——
...
#   (1) 有「## 修改记录」节：正文起点=修改记录表结束后首个 --- 的下一行；
#      表后无 --- 时（如 12 测试清单全文无 ---）=表结束后第一个非表格行；
#   (2) 无「## 修改记录」节（如 stub 追踪清单/v0.2.1 计划）：前 30 行内首个 --- 的下一行；
#   (3) 兜底=1（全扫，宁误扫不漏扫）。
