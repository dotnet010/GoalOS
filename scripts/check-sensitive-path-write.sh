...
#
# 事故史（2026-09-06 实证）：WinAC Boundary(2)测试直接 os.WriteFile(真实
# ~/.ssh/config)——毁损用户 SSH 配置（goalos-test 登录条目丢失）。
# 教训：测试=可能跑在任何人的机器上——夹具永不写真实用户既有文件。
#
# 规则（三条件同文件同满=红——反证迭代两轮实证：单行模式漏过变量间接形态，
# 纯构造匹配误伤分类断言）：
#   (1)文件构造敏感目录（".ssh"/".aws"/".gnupg" 字面量）
#   (2)文件出现敏感文件终段字面量（"config"/"id_*"/"credentials"/".gitconfig"/
#     ".netrc"/"known_hosts"）且该行无豁免标记（goalos- 夹具名 或 safefixture: 注释）
#   (3)文件含写入动词（os.WriteFile/Create/MkdirAll/Remove/RemoveAll）
