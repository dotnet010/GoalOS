...
func registerPlatformProvider(rb *runtimeBoundary, home string) {
	p := goalosruntime.NewWindowsJobProvider(home+"\\Goals", "\\tmp\\goalos")
	if err := p.Prepare(context.Background(), goalosruntime.RuntimePlan{PlanID: "daemon-boot", Tier: goalosruntime.TierRestricted}); err != nil {
		log.Printf(`{"level":"WARN","msg":"Step 7c: windows Provider Prepare 失败（诚实不注册）: %v"}`, err)
		return
	}
	if err := rb.registry.RegisterChecked(p); err != nil {
		log.Printf(`{"level":"WARN","msg":"Step 7c: windows Provider 注册被拒: %v"}`, err)
		return
	}
	log.Printf(`{"level":"INFO","msg":"Step 7c: windows-jobobject Provider 已注册（受限档 T1）"}`)
}
